...
for i in range(0,4):
    ...
    count_flux_upon_impact = np.genfromtxt("output/count_flux_upon_impact_"+str(int(index_grain_sizes[0]))+"_"+str(int(index_grain_sizes[1]))+".csv", delimiter=',')
    ...
    count_interp = np.interp(r_d, range_bounds_mid, total_count_flux_upon_impact, right = 0)
    sum_count_interp = sum_count_interp + count_interp
    
    
    #plt.semilogy(r_d/1000, energy_interp, linestyle = 'dashed', linewidth = 2, label = label_list[i])
    plt.semilogy(r_d, count_interp*0.0025, linestyle = 'dashed', linewidth = 2, label = label_list[i])
    #plt.semilogy(r_d, mass_interp, linestyle = 'dashed', linewidth = 2, label = label_list[i], zorder = (i+100) * 2)

plt.semilogy(r_d, sum_count_interp*0.0025, color = 'black', linewidth = 2, label = "Total Contribution")
#plt.semilogy(r_d/1000, sum_energy_interp, color = 'black', linewidth = 2, label = "Total Contribution")
#plt.semilogy(r_d, sum_mass_interp, color = 'black', linewidth = 2, label = "Total Contribution", zorder = 0)

plt.grid()
plt.legend(fontsize = 10)
plt.xlabel("Distance from Centerline (km)", fontsize = 16)
...
plt.savefig("output/impact_count_profile_combined_near.png", bbox_inches='tight',dpi=100)
