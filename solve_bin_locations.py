...
if(sol_ej.plot_only is False):
    ...
    for t in range(0,301):
        ...
        for i in range(sol_ej.index_grain_sizes[0],sol_ej.index_grain_sizes[1]):   
            ...
            proportion = sol_ej.dust_wt_pct[i-1]
            ...
            for j in range(0, np.size(u_ej_sim_particle_bounds_lower)):
                ...
                total_mass_excavated_particle_size = ejecta_mass_excavated[j] * sol_ej.dust_wt_pct[i-1]
...
if(sol_ej.plot_only):
    range_bounds_mid = np.genfromtxt("output/range_bounds_"+str(sol_ej.index_grain_sizes[0])+"_"+str(sol_ej.index_grain_sizes[1])+".csv", delimiter=",")
    mass_flux_upon_impact = np.genfromtxt("output/mass_flux_upon_impact_"+str(sol_ej.index_grain_sizes[0])+"_"+str(sol_ej.index_grain_sizes[1])+".csv", delimiter=',')
    momentum_flux_upon_impact = np.genfromtxt("output/momentum_flux_upon_impact_"+str(sol_ej.index_grain_sizes[0])+"_"+str(sol_ej.index_grain_sizes[1])+".csv", delimiter=',')
    energy_flux_upon_impact = np.genfromtxt("output/energy_flux_upon_impact_"+str(sol_ej.index_grain_sizes[0])+"_"+str(sol_ej.index_grain_sizes[1])+".csv", delimiter=',')
